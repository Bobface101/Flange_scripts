## For Mod F., Type 01 GOST flanges

import csv
import os

script_dir = os.path.dirname(os.path.abspath(__file__))
input_file = os.path.join(script_dir, 'Mod_F_Type_01_Data.csv')
...
with open(input_file, 'r') as f:
    ...
    for row in reader:
        ...
        """
        overlap = K[1] - F[1]
        if overlap > 0: # if the bore intersects the chamfer, extend the bore object line up to the intersection
            scr_lines.append("LINE")
            scr_lines.append(fmt(F))
            scr_lines.append(f"@{overlap},0")
            scr_lines.append("")
       ...
       #  D  -> K -> J -> I -> H
        ...
        # main symmetric
        symmetric_diameter_dim(Q, 6.5*SPACING)
        symmetric_diameter_dim(O, 7.5*SPACING)
        symmetric_diameter_dim(J, 8.5*SPACING)
        symmetric_diameter_dim(midpoint(D,F),9.5*SPACING)
        symmetric_diameter_dim(C,10.5*SPACING)
        ...
        # thickness b and h, and h1 depth (or h2 i cant remember )
        scr_lines.append("DIMLINEAR") 
        ...
        #attributes
        scr_lines.append(f"{flange[0:5]}, {flange[6:]} Type01 Mod.F - GOST 33259-2015")
        scr_lines.append(f"{flange[0:5]}, {flange[6:]} Тип01 Исп.F - ГОСТ 33259-2015")
        scr_lines.append(f"1:{dimscale}")    #scale
        scr_lines.append(f"{flange[2:5]}-{flange[8:]}-01-F-09G2S GOST 33259-2015")
    ...
    scr_lines.append("90")
    scr_lines.append("Mod. F Type 01")
    scr_lines.append("\n\n")
