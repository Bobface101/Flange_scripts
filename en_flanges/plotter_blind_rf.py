...
#start reading individual flanges here
    for col_name, d1 in row.loc["d1_PN6":"d1_PN25"].items():
        ...
        t1 = (pC[0]+1*dimscale,pC[1])
        if skip_centre_machining:
            t2 = (pC[0]+1*dimscale,sy+G_max/2)
        else:
            t2 = (pC[0]+1*dimscale,sy)
        """
        mappings:
        H -> unused
        L -> pA
...
# Collect ALL X and Y coordinates to find the true bounding box
        ...
        #attributes
        scr_lines.append(f"""EN 1092-1/05/B1/DN{DN}/{PN}/[material]""")
        scr_lines.append(f"""EN 1092-1/05/B1/DN{DN}/{PN}/[материал]""")
