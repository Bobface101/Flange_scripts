...
def show(pt):
    scr_lines.append("CIRCLE")
    scr_lines.append(fmt(pt))
    scr_lines.append("0.5")
...
START_DRAWING_POSITION = [0,-70000]
...
#start reading individual flanges here
    for col_name, d1 in row.loc["d1_PN6":"d1_PN25"].items():
        ...
        print(f"DN{DN} {PN} Wall thickness: {S}. pt diff: {pM[1]-pO[1]} ")
        show(pO)
        show(pM)
        """
        mappings:
    ...
    # Collect ALL X and Y coordinates to find the true bounding box
        ...
        scr_lines.append("0")
        scr_lines.append(f"""Weld Neck RF""")
        scr_lines.append("\n\n")
        ...
        #attributes
        scr_lines.append(f"""EN 1092-1/11/B1/DN{DN}/{PN}/[material]""")
        scr_lines.append(f"""EN 1092-1/11/B1/DN{DN}/{PN}/[материал]""")
