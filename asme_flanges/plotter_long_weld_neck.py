...
with open(input_file, 'r') as f:
    ...
    for row in reader:
        ...
        Y = O                 #just for display, this length is as required
        B = NPS*25.40         # for long weld neck standard is NPS to mm
        ...
        t1 = (sx, sy + X/2 + 3)
        t2 = (sx - 3, sy + X/2) # little inner chamfer
        t3 = (sx - 0.25*Y, sy + X/2)
        t4 = (sx - 0.25*Y, sy + B/2)
...
# Collect ALL X and Y coordinates to find the true bounding box
        ...
        line(pB,pC,pD,pE,pJ,t1,t2,t3,t4,pB)
        line(pF,pG,pH,pI,pF)
        line(pB,pA)
        line(t4,(sx - 0.25*Y, sy))
        line(pF, pE)
        line(pI,pJ)
        ...
        # main centerline
        cpB = conjugate(pB); ct4 = conjugate(t4)
        scr_lines.append("ZOOM")
        scr_lines.append("W")
        scr_lines.append(fmt((t4[0] - groove_margin, min(t4[1], ct4[1]) - groove_margin)))
        scr_lines.append(fmt((pB[0] + groove_margin, max(t4[1], ct4[1]) + groove_margin)))

        scr_lines.append("CENTERLINE")
        scr_lines.append(fmt(midpoint(pB, t4)))
        scr_lines.append(fmt(midpoint(cpB, ct4)))
        ...
        # main symmetric
        symmetric_diameter_dim(t4, -SPACING, (t4[0],sy))
        symmetric_diameter_dim(t3, -2*SPACING, (t4[0],sy))

        symmetric_diameter_dim(pC, SPACING, pA)
        symmetric_diameter_dim(midpoint(pF,pE),2*SPACING, pA)
        symmetric_diameter_dim(pG,3*SPACING, pA)
        ...
        temp = midpoint(t4,conjugate(t4))
        scr_lines.append(fmt(((temp[0]-4*SPACING),(temp[1])))) 
        ...
        scr_lines.append("DIMLINEAR")
        scr_lines.append(fmt(conjugate(pG))) #h dim
        scr_lines.append(fmt(conjugate(pC))) 
        ...
        # neck 
        scr_lines.append("DIMLINEAR")
        scr_lines.append(fmt((pH)))
        scr_lines.append(fmt((t3))) 
        scr_lines.append("T") # text edit 
        scr_lines.append(f"""As Required""") 
        temp = (midpoint((pH),(t3))[0],(pH)[1])
        ...
        #attributes
        scr_lines.append(f"""{NPS:g}", CL{Series:g}, RF, LWN, SCH STD, [material], B16.5""")
        scr_lines.append(f"""{NPS:g}", CL{Series:g}, RF, LWN, SCH STD, [материал], B16.5""")
    ...
    scr_lines.append("90")
    scr_lines.append("Long Weld Neck")
    scr_lines.append("\n\n")
